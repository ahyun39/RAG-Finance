...
정량 측정을 위한 평가용 골든셋 30문항을 구축했습니다(`eval/golden_set.json`, 2026-07-13). 법령 용어 질문 10개 / 구어체 질문 10개(생활법령정보 100문100답 기반) / 코퍼스에 답이 없어 거부가 정답인 질문 10개로 구성되며, Recall@5·MRR·거부 정확성 측정 스크립트는 작성 예정입니다.
...
| Context Precision (검색 정밀도) | 부분 충족 | 유사도 임계값 0.5 적용(2026-07-02), 정량 측정 없음 |
...
유사도 0.5 이상 청크만 LLM 컨텍스트에 전달되며, 관련 청크가 없으면 답변 생성 자체를 건너뜁니다(2026-07-02 적용). 다만 금융 법령 특성상 유사 표현이 다른 맥락에서 반복되어 의미상 관련 없는 조항이 임계값을 넘을 수 있고, 검색 품질을 정량 측정하는 골든셋 평가는 아직 없습니다.
